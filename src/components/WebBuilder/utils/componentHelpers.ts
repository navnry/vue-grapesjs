...
export function getChildren(compRef: any, events = cmpEvents) {
  ...
  const options = {
    modelOpts: {
      ...
      onDecouple: (cmp: any) => cmp.components._decouple(),
    },
    // Do not include components of type textnode in the tree
    alter: {
      method: 'filter',
      callback: (cmp: any) => cmp.get('type') !== 'textnode',
    },
  }
...
export function getAttributes(compRef: any) {
  return new Proxy(compRef, {
    get: (cmp: any, prop: string) => cmp.value.getAttributes()[prop],
    set: (cmp: any, prop: string, value: any) =>
      cmp.value.addAttributes({ [prop]: value }),
  })
}

// Get a reactive list of the classes on the component
export function getClasses(compRef: any) {
...
function getTraitValue(compRef: any) {
  return computed({
    get: () => compRef.value.getValue(),
    set: (val: any) => compRef.value.setTargetValue(val),
  })
}

// Get a reactive list of traits for the component type
export function getTraits(compRef: any) {
