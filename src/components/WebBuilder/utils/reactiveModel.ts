...
export default function reactiveModel(model: any, options: any = {}) {
  ...
  const proxy: Record<string, any> = shallowReactive({
  ...
  Object.keys(model.attributes).forEach((attr) => {
    if (typeof options.overwrites[attr] !== 'function') {
      const value = computed({
        ...
        set: (val: any) => modelRef.value.set(attr, val),
      })
      proxy[attr] = value
    }
  })
  ...
  options.events.forEach((evt: any) => model.on(evt, proxy._triggerModel))

  // Add method to proxy to remove triggers from model
  function decouple() {
    if (options.onDecouple) options.onDecouple(proxy)
    options.events.forEach((evt: any) => model.off(evt, proxy._triggerModel))
  }
