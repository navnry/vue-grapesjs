 ...
 */
export default function useStyleProps(grapes: any) {
  // Ensure GrapesJs is not yet initialised
  if (grapes.initialized)
    throw new Error(
      'useStyleProps must be executed before GrapesJs is initialised (onMount where useGrapes is executed)'
    )
    ...
    grapes.onInit((editor: any) => {
      ...
      styles.sectors = reactiveCollection(editor.StyleManager.getSectors(), {
        modelOpts: {
          overwrites: {
            properties: (cmp: any) =>
              reactiveCollection(cmp.value.get('properties')),
          },
        },
      })
      ...
      styles.getBuiltInAll =
        editor.StyleManager.getBuiltInAll.bind(editor.StyleManager)
