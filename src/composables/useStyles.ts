...
export default function useStyles(grapes: any) {
  ...
  if (!grapes._cache.styles) {
    ...
    grapes.onInit((editor: any) => {
      ...
      function proxyStyle(modelRef: any) {
        return new Proxy(modelRef, {
          // Get a style from Rule model
          get: (model: any, style: string) => model.value.getStyle(style),
          // Add new style to styles object and set new styles on Rule
          set: (model: any, style: string, val: any) => {
      ...
      function updateSelected() {
        ...
        if (selected !== cm.selected.rule._model) {
          ...
          if (selected) {
            cm.selected.rule = reactiveModel(selected, {
              overwrites: { style: proxyStyle },
            })
