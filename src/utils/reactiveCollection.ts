...
function getCachedProxy(collection: any[], options: any, model: any) {
  return (
    collection.find((cache: any) => cache.cid === model.cid) ??
...
function mapReturnModels(fn: any, key: string, cache: any[], options: any) {
  if (returnsModels.indexOf(key) === -1) return fn
  return function (this: any) {
    const res = fn.apply(this, arguments as any)
    if (Array.isArray(res))
      return res.map(getCachedProxy.bind(null, cache, options))
    return getCachedProxy(cache, options, res)
  }
}

// Provide an array with all functions of the input collection assigned to it
function bindCollectionFunctions(collection: any, options: any) {
  const res: any[] = []
...
function updateCollection(proxy: any[], options: any, collection: any) {
  const cache = [...proxy]
 ...
 */
export default function reactiveCollection(collection: any, options: any = {}) {
